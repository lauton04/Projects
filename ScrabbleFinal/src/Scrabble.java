...
public class Scrabble {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					window.frmScrabblebyNorthWest.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		frmScrabblebyNorthWest = new JFrame();
		Dimension dim = Toolkit.getDefaultToolkit().getScreenSize();
		frmScrabblebyNorthWest.setBounds(10, 10, dim.width / 2, dim.height - 50);
		frmScrabblebyNorthWest.setLocation(dim.width / 2 - frmScrabblebyNorthWest.getSize().width / 2, dim.height / 2 - frmScrabblebyNorthWest.getSize().height / 2);
		frmScrabblebyNorthWest.setResizable(false);
		frmScrabblebyNorthWest.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmScrabblebyNorthWest.getContentPane().setLayout(null);
		
		lblEndScreen = new JLabel("");
		lblEndScreen.setIcon(new ImageIcon("endScreen.jpg"));
		lblEndScreen.setBounds(12, 0, 655, 546);
		frmScrabblebyNorthWest.getContentPane().add(lblEndScreen);
		lblEndScreen.setVisible(true);

		boardPanel = new JPanel();
		boardPanel.setBounds(10, 11, dim.width / 2 - 30, dim.height - 300);
		frmScrabblebyNorthWest.getContentPane().add(boardPanel);
		boardPanel.setLayout(new GridLayout(xDimension, yDimension));

		JPanel panel_1 = new JPanel();
		panel_1.setBounds(103, 490, 472, 50);
		frmScrabblebyNorthWest.getContentPane().add(panel_1);
		...
		lblPlayerScore_1.setBounds(49, 618, 106, 14);
		frmScrabblebyNorthWest.getContentPane().add(lblPlayerScore_1);

		btnPlayWord = new JButton("Play word");
		btnPlayWord.addActionListener(new BtnPlayWordActionListener());
		btnPlayWord.setBounds(49, 568, 116, 23);
		frmScrabblebyNorthWest.getContentPane().add(btnPlayWord);
		...
		txtPlayer1Score.setBounds(59, 643, 86, 20);
		frmScrabblebyNorthWest.getContentPane().add(txtPlayer1Score);
		...
		btnSkipTurn.setBounds(270, 568, 136, 23);
		frmScrabblebyNorthWest.getContentPane().add(btnSkipTurn);
		...
		txtPlayer2Score.setBounds(295, 643, 86, 20);
		frmScrabblebyNorthWest.getContentPane().add(txtPlayer2Score);

		btnUndo = new JButton("Undo Move");
		btnUndo.addActionListener(new ButtonUndoActionListener());
		btnUndo.setBounds(506, 568, 136, 23);
		frmScrabblebyNorthWest.getContentPane().add(btnUndo);

		lblPlayerScore_2 = new JLabel("Player 2 score");
		lblPlayerScore_2.setHorizontalAlignment(SwingConstants.CENTER);
		lblPlayerScore_2.setBounds(285, 618, 106, 14);
		frmScrabblebyNorthWest.getContentPane().add(lblPlayerScore_2);
		...
		currentTurn.setBounds(531, 643, 81, 20);
		frmScrabblebyNorthWest.getContentPane().add(currentTurn);

		JLabel lblCurrentTurn = new JLabel("Current turn");
		lblCurrentTurn.setHorizontalAlignment(SwingConstants.CENTER);
		lblCurrentTurn.setBounds(521, 618, 106, 14);
		frmScrabblebyNorthWest.getContentPane().add(lblCurrentTurn);
		
		menuBar = new JMenuBar();
		frmScrabblebyNorthWest.setJMenuBar(menuBar);
		...
		// creates the gridLayout "field" to make the frame of the scrabble board
		...
		for (int i = 0; i < 7; i++) {
			...
			frmScrabblebyNorthWest.setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource("Icon.png")));
		}
	}
	...
	public void swapTurn() {
		...
		if (playerOnesTurn == true) {
			...
			JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "Player one's turn!");
		} else {
			currentPlayer = playerTwo;
			pouch.displayLettersInTrayfield(playerTwo.tray);
			currentTurn.setText("Player Two");
			JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "Player two's turn!");
		}
	}
	...
	private class TextFieldActionListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent arg0) {
			if (letter.equals("")) {
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "Please select a letter");
			} else if (field[xPos][yPos].getBackground() == Color.LIGHT_GRAY) {
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "Please put the letter in a different spot");
			} else {
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (int i = 0; i < temp.size(); i++) {
				try {
					...
					JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "There was an error");
					System.exit(-1);
				}
			}
			// do checks
			if (word.isEmpty()) {
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "Please play a word");
			} else if (turn == 0 && field[7][7].getText().equals("2W")) {
				// System.out.println("got there");
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest,
				...
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "The word does not exist. Please try again!");
				btnUndo.doClick();
			} else {
	...
	private class MntmRestartActionListener implements ActionListener {
		...
		public void dispose() {
			frmScrabblebyNorthWest.dispose();
		}
	}
	...
	private class MntmEndGameActionListener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			...
			ImageIcon Image2 = new ImageIcon(this.getClass().getResource("endScreen.jpg"));

			
			if (player1score > player2score) {
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "PLAYER 1 IS THE WINNERRRR!!!!!");
			} else if (player2score > player1score) {
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "PLAYER 2 IS THE WINNERRRR!!!!!");
			} else
				JOptionPane.showMessageDialog(frmScrabblebyNorthWest, "THE GAME IS A TIE!!!!!");
